...
class Line:
	...
	def __init__(self,name,center,waves,spec):
		...
		self.flux = scipy.integrate.quad(gauss_fwhm, min(waves_cut)-50, max(waves_cut)+50, args=(self.A,self.x0,self.fwhm))[0]
                """JS: You might consider wrapping some of these long
                lines. For example, ::

                  self.flux = scipy.integrate.quad(
                          gauss_fwhm, min(waves_cut)-50, max(waves_cut)+50,
                          args=(self.A,self.x0,self.fwhm))[0]
...
def plot():
	...
	global o4363,o4959,o5007,s6716,s6731
        """JS: There's almost always a better way than to declare a
        variable as global. Why not just return the `lines` list? Or,
        separate the creation of your `Line` instances from your plotting
        routine.
