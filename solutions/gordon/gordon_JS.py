...
"""JS: If you like making classes, I'd get into the habbit of using
"new-style" classes. The most useful feature of new-style classes, IMO, is
the ability to define properties using the ``@property`` decorator, for
which you can write getters and setters to do whatever you want (basically
a "smart" attribute). The only syntactic difference between old- and
new-style classis is a delclaration of what your class inherits from
('object' being the generic option, but you can subclass just about
anything such as arrays or dictionaries). In this case::

  class LineFeature(object):
      # etc.
...
class LineFeature:
    ...
    def __init__(self, name, center, width, waves, flux,resample=None):
        ...
        """JS: maybe this? ::
          ...
          self.waves,self.continuum,self.flux = self.find_continuum(
                  waves, flux, width)

        or::

          result = self.find_continuum(waves, flux, width)
          self.waves, self.continuum, self.flux = result
...
def integrate(x,y):
    ...
    dx = np.diff(x)  """JS: cool trick! I didn't know you could do this."""
...
def get_1Dspec(filename):
    ...
    data = np.array(zip(waves,ydata),dtype=[('waves',np.float),
                                            ('flux',np.float)])
    """JS: Can also zip names with dtypes::

      names, dtypes = ['waves', 'flux'], ['float', 'float']
      data = np.array(zip(waves, ydata), dtype=zip(names, dtypes))
